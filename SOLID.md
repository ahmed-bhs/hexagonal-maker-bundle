...
#### Violation (Architecture traditionnelle)
```php
// Application Layer dépend DIRECTEMENT de l'infrastructure
class RegisterUserService
{
    public function __construct(
        private EntityManagerInterface $em,        // Dépendance concrète Doctrine
        private MailerInterface $mailer,           // Dépendance concrète Symfony Mailer
        private LoggerInterface $logger            // Dépendance concrète
    ) {}
    ...
    public function register(string $email, string $password): void
    {
        ...
        $this->em->persist($user);   // Couplé à Doctrine
        $this->em->flush();

        $this->mailer->send(...);    // Couplé à Symfony Mailer
    }
}
```

**Problèmes:**
- Impossible de changer Doctrine pour autre chose sans modifier le code
- Impossible de tester sans une vraie DB et un vrai mailer
- Le domaine dépend de l'infrastructure (**inversion incorrecte**)

#### Avec Architecture Hexagonale (DIP Correct)
```php
// Domain Layer - Définit l'abstraction (PORT)
namespace App\User\Domain\Port;

interface UserRepositoryInterface  // ← Abstraction définie dans le DOMAIN
{
    public function save(User $user): void;
    public function findById(UserId $id): ?User;
}

// Application Layer - Dépend de l'abstraction du DOMAIN
namespace App\User\Application;

class RegisterUserHandler
{
    public function __construct(
        private UserRepositoryInterface $repository  // ← Dépend de l'abstraction
    ) {}

    public function __invoke(RegisterCommand $cmd): void
    {
        $user = new User(...);
        $this->repository->save($user);  // Ne sait pas comment c'est sauvegardé
    }
}

// Infrastructure Layer - IMPLÉMENTE l'abstraction du domain
namespace App\User\Infrastructure\Persistence\Doctrine;

class DoctrineUserRepository implements UserRepositoryInterface  // ← Implémente
{
    public function __construct(
...
- L'**Infrastructure** dépend du **Domain** (elle implémente les ports)
- Changement d'infrastructure = créer un nouvel adapter, **aucune modification** du domain/application
